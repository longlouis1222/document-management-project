...
import modelData from './TopicStudentSuggestModel'

const defaultFilter = DataService.defaultFilter

const router = useRouter()
const moduleName = 'Đề tài đề xuất'
    ...
    if (valid) {
      try {
        if (viewMode.value === 'create') {
          ...
          const topicApiRes = await StudentApi.update(formData.value)
          if (topicApiRes.status === 200) {
...
const getList = async () => {
  ...
  const topicApiRes = await StudentApi.getListTopicSuggest(filter)
  if (topicApiRes.status === 200) {
      ...
      <template #header>
        <div class="card-header">
          <div class="d-flex justify-content-between">
            <h4>Danh sách đề tài đề xuất</h4>
            <div>
